;Кулешова Полина БПИ197
;Вариант 12
;Разработать программу, вычисляющую с  помощью степенного ряда с точностью
;не  хуже 0,1% значение функции 1/e^х   для  заданного параметра x (использовать FPU)
...
start:
  ...
  fld qword[x]
  fcom  qword[right] ;check is x more than 10
  ...
  ja exception
  fcomp  qword[left] ;check is x less than -10
  fstsw ax ;saves the current value of the SR register
  sahf ;copy the contents of the AH register into the lower 8 bits of the flags register
  jb exception
;teylor series
...
.end:
  ...
  jmp exit
 exception:
    invoke printf, exceptionr ;if x out of range
 exit:
    ret


section '.data' data readable writable
  left dq -10.0 ;lower limit of range
  right dq 10.0 ;upper limit of range
  a dq -1.0    ;sign
  x dq 0.0   ;value
  ...
  exceptionr db 'exception. x should be between -10 and 10',10,0 ;previous-previus sum
